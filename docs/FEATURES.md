...
| F-0.5b | Open / switch / restore / create additional vaults + recent list (Settings + Unlock) | 0.5b / 5.x | `done` | `test/vault_switch_service_test.dart`, `test/vault_backup_service_test.dart`, `test/recent_vault_store_test.dart` |
...
| F-5.2 | Backup / export + restore-from-backup (copy to Documents, then open) | 5.2 | `done` | `test/vault_backup_service_test.dart`, `test/register_csv_exporter_test.dart` |
